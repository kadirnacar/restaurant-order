...
class HeaderTitleComp extends Component<Props, HeaderTitleState> {
    ...
    render() {
        return (
            <Text style={[this.props.style, { color: "#fff", fontWeight: "bold", fontSize: 20 }]}>
                {this.props.Department.current ? this.props.Department.current.DEPARTMENTNAME : "Electra POS"}</Text>
        )
    }
}
